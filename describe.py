...
class DFSummary(object):
    ...
    def summary(self, **kwargs):
        return pd.concat([self.df.describe(**kwargs), self.columns_stats], sort=True)[self.df.columns]
    ...
    def _get_columns_info(self, stats):
        ...
        column_info[self.TYPE_NUMERIC] = pd.Index([c for c in rest_columns
                                                   if pd_types.is_numeric_dtype(self.df[c])])
        ...
        column_info[self.TYPE_DATE] = pd.Index([c for c in rest_columns
                                                if pd_types.is_datetime64_dtype(self.df[c])])
        rest_columns = self.get_columns(
